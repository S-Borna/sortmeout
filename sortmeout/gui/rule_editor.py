...
ACTION_TYPES = [
    ...
    ("extract", "Extract Archive", "Extract ZIP/TAR contents"),
    ("sort_into_subfolder", "Sort into Subfolder", "Sort into dated/named subfolder"),
    ("make_alias", "Create Alias", "Create a macOS Finder alias"),
    ("toggle_lock", "Toggle Lock", "Lock or unlock the file"),
    ("toggle_extension", "Toggle Extension", "Show or hide file extension"),
    ("import_to_photos", "Import to Photos", "Import into Photos app"),
    ("import_to_music", "Import to Music", "Import into Music app"),
    ("run_shell", "Run Shell Script", "Execute a shell command"),
    ("run_applescript", "Run AppleScript", "Execute AppleScript"),
    ("run_automator", "Run Automator Workflow", "Run an Automator workflow"),
...
if HAS_APPKIT:
    ...
    class ActionRowView(NSView):
        ...
        def _update_for_action_type(self, idx: int):
            ...
            needs_path = action_id in ("move", "copy", "open_with", "run_automator", "make_alias", "extract")
            ...
            placeholders = {
                ...
                "extract": "Destination folder (default: same location)",
                "sort_into_subfolder": "Pattern (e.g., {year}/{month})",
                "make_alias": "Folder to create alias in",
                "toggle_lock": "lock / unlock / toggle",
                "toggle_extension": "show / hide / toggle",
                "import_to_photos": "Album name (optional)",
                "import_to_music": "(no value needed)",
                "run_shell": "Shell command",
                "run_applescript": "AppleScript code",
                "run_automator": "Workflow file path",
                "run_shortcut": "Shortcut name",
                "notify": "Notification message",
            }
            self.value_field.setPlaceholderString_(placeholders.get(action_id, "Value"))
            no_value_types = ("trash", "import_to_music")
            self.value_field.setEnabled_(action_id not in no_value_types)
        ...
        def get_action_data(self) -> Dict[str, Any]:
            ...
            if action_id in ("move", "copy"):
            ...
            elif action_id in ("add_tags", "remove_tags", "set_tags"):
                ...
                if value:
                    params["destination"] = value
            elif action_id == "sort_into_subfolder":
                params["pattern"] = value or "{year}/{month}"
            elif action_id == "make_alias":
                params["destination"] = value
            elif action_id == "toggle_lock":
                lock_map = {"lock": True, "unlock": False}
                params["lock"] = lock_map.get(value.lower())
            elif action_id == "toggle_extension":
                show_map = {"show": True, "hide": False}
                params["show"] = show_map.get(value.lower())
            elif action_id == "import_to_photos":
                if value:
                    params["album"] = value
                ...
                params["script"] = value
            elif action_id == "run_automator":
                params["workflow"] = value
    ...
    class RuleEditorWindowController(NSObject):
        ...
        def _load_existing_rule(self):
            ...
            for cond in rule.get("conditions", []):
                ...
                self._set_condition_values(row, cond)

            # Load actions
            for action in rule.get("actions", []):
                self.addAction_(None)
                row = self._action_rows[-1]
                self._set_action_values(row, action)
        ...
        @python_method
        def _set_condition_values(self, row, cond: Dict):
            """Set a condition row's values from saved data."""
            attr = cond.get("attribute", "")
            operator = cond.get("operator", "")
            value = cond.get("value", "")

            # Find and select the attribute
            for i, (attr_id, _, _) in enumerate(CONDITION_ATTRIBUTES):
                if attr_id == attr:
                    row.attribute_popup.selectItemAtIndex_(i)
                    # Trigger attribute change to update operators
                    row.attributeChanged_(row.attribute_popup)
                    break
            ...
            for i, (op_id, _) in enumerate(ops):
                if op_id == operator:
                    row.operator_popup.selectItemAtIndex_(i)
                    break

            # Set the value
            if isinstance(value, list):
                row.value_field.setStringValue_(", ".join(str(v) for v in value))
            else:
                row.value_field.setStringValue_(str(value) if value else "")

        @python_method
        def _set_action_values(self, row, action: Dict):
            """Set an action row's values from saved data."""
            action_type = action.get("action_type", "")
            params = action.get("params", {})

            # Find and select the action type
            for i, (act_id, _, _) in enumerate(ACTION_TYPES):
                if act_id == action_type:
                    row.action_popup.selectItemAtIndex_(i)
                    row._update_for_action_type(i)
                    break

            # Set the value based on action type
            value = ""
            if action_type in ("move", "copy"):
                value = params.get("destination", "")
            elif action_type == "rename":
                value = params.get("pattern", "") or params.get("new_name", "")
            elif action_type in ("add_tags", "remove_tags", "set_tags"):
                ...
                value = params.get("format", "zip")
            elif action_type == "run_shell":
                value = params.get("command", "") or params.get("script", "")
            elif action_type == "run_applescript":
                value = params.get("script", "")
            elif action_type == "run_shortcut":
                value = params.get("shortcut_name", "") or params.get("shortcut", "")
            elif action_type == "notify":
                value = params.get("message", "")
            elif action_type == "sort_into_subfolder":
                value = params.get("pattern", "") or params.get("subfolder", "")
            elif action_type == "run_automator":
                value = params.get("workflow", "") or params.get("path", "")

            row.value_field.setStringValue_(str(value))
